...
#    x_test[i] = np.fromstring(data_test['pixels'][32298+i], dtype=int, sep=' ')
    x_test[i] = np.fromstring(data_test['pixels'][5+i], dtype=int, sep=' ')
...
#Replicate the image
## Resize The images :
img_rows1, img_cols1, channels = 224, 224,3

X_train = np.zeros((y_train.shape[0],img_rows1, img_cols1, channels))
x_train_rev = np.zeros((y_train.shape[0],img_rows1, img_cols1, channels))
for i in range(y_train.shape[0]):
...
X_test = np.zeros((y_test.shape[0],img_rows1, img_cols1, channels))
x_test_rev = np.zeros((y_test.shape[0],img_rows1, img_cols1, channels))
...
plt.figure(2)
plt.imshow(x_train_rev[0].reshape((224,224,3)))

# Some preprocessing
input_shape = (img_rows1, img_cols1, 3)
